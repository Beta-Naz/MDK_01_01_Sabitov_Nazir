...
namespace Batlle_Сабитов.Classes.Metod
{
    public class SpawnMonsters
    {
        ...
        {
            if (Locations.CreateLocations.LastLocationSourse == Locations.RepoAllLocations.AllLocations()[5].BackgroundLocations)
            {
                Batlle.init.SpawnMonsters.Children.Clear();
                AlwaysMonsters.WhatSpawnMonsters.Clear();
                AlwaysMonsters Boss = Classes.RepoAlwaysMonsters.AllMonster()[RepoAlwaysMonsters.AllMonster().Count - 1];
                Turn.ResetTurns(1);
                AlwaysMonsters.WhatSpawnMonsters.Add(Boss);
                Batlle.init.SpawnMonsters.Children.Add(new Elements.Boss(Boss));
            }
            else
            {
        ...
        public static void ResetWhatSpawnMosters(string nameLocation)
        {
            ...
            foreach (Locations.CreateLocations locations in Locations.RepoAllLocations.AllLocations())
            {
